...
class UsersTestCase(TestCase):
    ...
    def test_reconstructing_removed_user_group_relationship(self) -> None:
        ...
        user_group_relationship_content_type = ContentType.objects.get_for_model(
            UserGroupRelationship,
        )
        ...
        object_delta = ObjectDelta.objects.filter(
            ...
            recovered_snapshot=ObjectSnapshot(UserGroupRelationship)
        ).order_by(
            '-pk',
        ).first()
        ...
        recovered_user_group_relationship = getattr(object_delta, 'recovered_snapshot', None)
        self.assertIsNotNone(recovered_user_group_relationship)
        self.assertEqual(user.groups.count(), 0)

        recovered_user_group_relationship = cast(UserGroupRelationship, recovered_user_group_relationship)
        recovered_user_group_relationship.save()
        self.assertEqual(user.groups.count(), 1)

        self.assertEqual(recovered_user_group_relationship.pk, removed_user_group_relationship_pk)
